...
Distmethod = 'rJSD'
Validmethod = 'CH'
if(length(args)==6){Distmethod = args[6]}
if(length(args)==7){Validmethod = args[7]}
...
cluster_plot <- function(data, file_name_rt, k , percent) {

        data.dist <- dist(data,method=Distmethod)   # generate distance matrix

        if(k == 0){
            k <- OptimizeK(data,method = Validmethod, 1, Krange, file_name= file_name_rt)
        }else{
        ...
        for (i in 1:nrow(data.sample)) {
                if (strsplit(rownames(data.sample)[i], "")[[1]][1] == "H") {data.sample[i, n+1] <- 19}
                if (strsplit(rownames(data.sample)[i], "")[[1]][1] == "D") {data.sample[i, n+1] <- 8}
        }
        ...
        all_colors <- c("coral3", "aquamarine3", "antiquewhite4", "darkolivegreen3", "dodgerblue2", "firebrick4", "gray33")
        clu_colors <- all_colors[1:k]
        ...
        legend("topleft", title = "Races Legends", pch = c(19, 8), legend = c("H","D"),, cex = 1)
...
rm_cluster_plot <- function(file_name, k , percent, rm_n) {
        data <- read.table(file_name, header=T, row.names=1, dec=".")   # read abundance matrix
        file_name_rt <- strsplit(file_name, ".txt")
        data <- noise.removal(data, percent)  # genertae denoized data
        for (i in 1:(rm_n+1)) {
                cluster_plot(data, file_name_rt, k , percent)
                data <- rm_top_class(data)
                file_name_rt <- paste(file_name_rt, "_rm", i, sep = "")       
        }     
}

rm_cluster_plot(file_name, k , percent, rm_n)
