...
data <- read.table(dataFile,header=T,sep = "\t",row.names=1);
data <- data[,-ncol(data)];
myFun=function(x){x/sum(x)};
Abundance <- data.frame(apply(data,2,myFun));

top <- 20;

grpInfo <- read.table(groupFile,header=T);

write.table(round(Abundance,6),file=paste(outDir,"/",SamID,".",FunID,".abundance.txt",sep=""),quote = F,sep="\t",col.names=NA);
...
pdf(paste(outDir,"/",SamID,".",FunID,".abundance.boxplot.pdf",sep=""),width=12,height=7.5);
