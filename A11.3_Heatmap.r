...
setwd(arg3[1])
######################	参数
arg1 <- c("CC","CP","HC","HP","HL","HM","HH")	###Heatmap 
arg2 <- c("HC","CC","HL","HM","HH")	## Grid;第一位必须是基准;第二位必须是正常组，后面是给药组（或者相反）
#########	P值
P <- 0.01
###########	自定义函数区
### 判断形状
...
gut <- read.csv(file="03_gut.csv",header=T,row.names=1)
abundance <- read.csv(file="03_abundance.csv",header=T,row.names=1)
...
## 挑选所选中的OTU的P值
all_p <- read.csv(file="03_all_p.csv",header=T,row.names=1)
