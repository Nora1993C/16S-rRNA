...
Alpha <- as.matrix(data.frame(read.table(dataFile,header=T,row.names = 1)));

groupname <- c();
for(i in 1:length(rownames(Alpha)))
{
	groupname <- append(groupname,as.character(grpInfo[grep(rownames(Alpha)[i],grpInfo[,1]),grp_col]));
}
...
pdf(paste(outDir,"/",SamID,".",GID,".",MetID,".final.otu.barplot.pdf",sep=""),width=4,height=4)
...
bar.pos <- barplot(Alpha.Group.mean,border=NA,col=rainbow(length(Groups)),names.arg=Groups,ylim=c(0,y.lim),ylab=paste(MetID,"Index",sep=" "),cex.names=1.0,cex.lab=1.0,cex=1.2)
